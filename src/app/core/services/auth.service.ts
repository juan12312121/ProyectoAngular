...
deleteUser(id: number): Observable<any> {
  const url = `https://backend-2-f5qo.onrender.com/usuario/users/${id}`;  // Cambia aquí para incluir '/usuario'
...
verHistorial(usuarioId: number): Observable<any> {
  ...
  const url = `https://backend-2-f5qo.onrender.com/usuario/historial/${usuarioId}`;
...
getUserById(id: number): Observable<any> {
  const url = `https://backend-2-f5qo.onrender.com/usuario/users/${id}`;
...
register(
  ...
  getUserProfile(): Observable<any> {
    ...
    const url = `https://backend-2-f5qo.onrender.com/usuario/users/${userId}`;
